 ...
 *
 * <p>Nodes are identified by a number (their index) and assumed to be indexed
 * from 1.
 *
 * <p>
 * We store the data into a (n + 1) * (n + 1) adjacency matrix where n is the
 * number of nodes. The first row (row 0) of the matrix is reserved to store the
 * visit status of each node in our program in particular, and it will come in
 * handy in the graphics code. The real adjacency matrix start from (1, 1) to
 * (n + 1, n + 1). Value <code>true</code> corresponds to a connection between
 * the node identified by the row number and the node identified by the column
 * number, while <code>false</code> means disconnection. Since the graph is
 * undirected, it is symmetric from (1, 1) to (n + 1, n + 1) (the first row and
 * first column are reserved for any data the programmer wishes to store).
 *
 * @author Duong Nguyen
...
class NetworkGraph {
   ...
   * @param x
   * @param y
   * @return <code>true</code> if the two nodes are connected, otherwise
   * <code>false</code>.
   */
  ...
  void getConnectedNodes(int node, ArrayList<Integer> connectedNodes) {
    ...
    for (int i = node + 1; i < data.length; i++)
   ...
   *
   * <p>
   * This assumes that a call to <code>collectiveInfluence(int, int)</code> has
   * been made.
   *
   * @param node
  ...
  /**
   * Isolates the node with the highest degree or the highest collective
   * influence.
   *
  ...
  IsolationInfo isolate(boolean useDegree, int radius, boolean trace) {
    IsolationInfo isolationInfo = (useDegree) ? degreeIsolate() : collectiveInfluenceIsolate(radius, true);
   ...
   * @param computeCollectiveInfluences Whether or not collective influences
   * have to be computed for the given radius before isolation can begin.
   */
  IsolationInfo collectiveInfluenceIsolate(int radius, boolean computeCollectiveInfluences) {
    ...
    for (int i = 1; i < shortestPaths.length; i++) {
      if (shortestPaths[i] <= radius)
        collectiveInfluences[i] = collectiveInfluence(i, radius);
    }
