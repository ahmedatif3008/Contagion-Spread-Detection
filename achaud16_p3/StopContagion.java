 ...
 *
 * <p>
 * Its significant component, the <code>NetworkGraphCanvas</code>, is
 * implemented in its own file. The constructor prepares the rest of the UI and
 * implements their functionality. The program does not always have to be
 * started in GUI mode—the --no-gui flag can be invoked for quick testing.
 */
class StopContagion extends JFrame implements ActionListener {
  ...
  StopContagion(NetworkGraph network, int numNodes, boolean useDegree, int radius, boolean trace) {
    ...
    this.canvas.onScreenMessage = (this.numNodes == 0) ? "Not inoculating, view only" : "Click the Isolate button to begin the inoculation process";
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (source == this.isolateButton) {
      if (this.step < this.numNodes) {
        ...
        this.canvas.onScreenMessage = String.format("Isolated node %d with %s %d", isolationInfo.node, isolationInfo.unit, isolationInfo.influence);
        this.canvas.update(this.canvas.getGraphics());
